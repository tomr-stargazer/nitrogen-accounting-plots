...
# Abundance value inspired by 
def h13cn_Tex_vs_density(save=True, column=1e12, abundance=1e-8, print_timing=False):
    ...
    ax.text(1.5, 20, "H$^{13}$CN", fontsize=18, family='serif')
    ax.text(2.35, 15, "$J = 1 - 0$", fontsize=16, family='serif', color='b')
    ax.text(3.6, 18, "$J = 2 - 1xxx$", fontsize=16, family='serif', color='g')
    ax.text(4.1, 10, "$J = 3 - 2$", fontsize=16, family='serif', color='r')

    ax.minorticks_on()

    if save:
        fig.savefig("h13cn_Tex_vs_density.pdf")
...
def h13cn_fc_versus_density(save=True, print_timing=False, abundance=1e-8):
    ...
    density_array = np.logspace(4, 10, 100)
    temperature_list = [30, 50, 100]
    ...
    f_c_array_list = [f_c_array_100K, f_c_array_30K, f_c_array_50K]
    ...
    for j, temp in enumerate(temperature_list):
        ...
        for i, rho in enumerate(density_array):
            ...
            f_c = correction_factor_given_radex_table(new_T, h13cn_J_upper_list)

            f_c_array_list[j][i] = f_c
            
    ax.plot(np.log10(density_array), f_c_array_100K, '-', lw=1.5)
    ...
    ax.text(4.5, 2.8, "H$^{13}$CN", fontsize=18, family='serif')
    ax.text(6, 1.55, r"$T_k = 50\ \rm{K}$", fontsize=16, family='serif', color='r')
    ax.text(6, 2.2, r"$T_k = 30\ \rm{K}$", fontsize=16, family='serif', color='g')
    ax.text(6, 2.8, r"$T_k = 100\ \rm{K}$", fontsize=16, family='serif', color='b')
    ax.text(2.5, 2.5, r"$J_u=1,\ 3,\ 4,\ 6,\ 7,\ 8,\ 9,\ 10$ OBSERVED", fontsize=14, family='serif')

    ax.minorticks_on()

    if save:
        fig.savefig("h13cn_fc_vs_density.pdf")
